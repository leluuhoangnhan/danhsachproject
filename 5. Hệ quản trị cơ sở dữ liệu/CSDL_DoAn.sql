...
ADD IMG VARCHAR(300) DEFAULT ('LeLuuHoangNhan.png')
...
BEGIN
	...
	SET @SQL='BCP "SELECT * FROM [ADMIN/SQLEXPRESS].[DB_QLDA].[DBO].[CHUDAUTU]" QUERYOUT "E:\CDT.TXT" -c -T -t; -S ADMIN/SQLEXPRESS -U sa -P 1321'
...
---KHI TẠO 1 NGƯỜI DÙNG MẶC ĐỊNH (ĐĂNG KÍ Ở FROM ĐĂNG NHẬP) -->THÌ ĐƯỢC SẾP VÀO NHÓM QUYỀN "THÔNG THƯỜNG"
...
--XEM CÁC TÀI KHOẢN LOGIN HIỆN CÓ
GO
SELECT * 
FROM master.sys.sql_logins;


--XEM CÁC TÀI KHOẢN USER HIỆN CÓ
GO
SELECT *
FROM DB_QLDA.sys.database_principals;


--XEM CÁC ROLE HIỆN CÓ TRÊN DATABASE....
GO
SELECT *
FROM DB_QLDA.sys.database_principals WHERE type='R';


--XEM SỐ LƯỢNG THÀNH VIÊN HIỆN CÓ TRONG ROLE.... TRÊN DATABASE....
GO
SELECT principal_id AS N'SỐ LƯỢNG THÀNH VIÊN NHÓM QUẢN TRỊ'
FROM DB_QLDA.sys.database_principals WHERE type='R' AND name='Quan_tri';




-----------------------------BỔ SUNG THIẾU SÓT SAU BÁO CÁO------------------------------
GO
SELECT * FROM CHUDAUTU

GO
UPDATE CHUDAUTU
SET IMG = 'LeLuuHoangNhan.png'

UPDATE CHUDAUTU
SET IMG='Capitaland.png'
WHERE TENCDT=N'CAPITALAND' 

UPDATE CHUDAUTU
SET IMG='Khangdien.png'
WHERE TENCDT=N'KHANG ĐIỀN' 

UPDATE CHUDAUTU
SET IMG='AnGiaHung.png'
WHERE TENCDT=N'AN GIA HƯNG 01' 

UPDATE CHUDAUTU
SET IMG='HongKongLand.png'
WHERE TENCDT=N'PHƯỚC THÀNH' 




--HÀM TRẢ VỀ SỐ LƯỢNG DỰ ÁN CDT ĐÃ THAM GIA, THAM SỐ TRUYỀN VÀO LÀ MACDT (SỬ DỤNG CURSOR)
GO
...
--TẠO QUYỀN
GO
CREATE PROC TT_TAO_PHANQUYEN @TEN NVARCHAR(50), @QUYEN VARCHAR(50), @BANG VARCHAR(50)
AS
BEGIN
	IF(@QUYEN='ALL')
	BEGIN
		INSERT INTO PHANQUYEN
		VALUES(@TEN, 'INSERT', @BANG),
			(@TEN, 'UPDATE', @BANG),
			(@TEN, 'DELETE', @BANG),
			(@TEN, 'SELECT', @BANG)
	END
	...
	BEGIN
		INSERT INTO PHANQUYEN
		VALUES(@TEN, @QUYEN, @BANG)
	END
END

DROP PROC TT_TAO_PHANQUYEN


--XÓA QUYỀN
GO
CREATE PROC TT_XOA_PHANQUYEN @TEN NVARCHAR(50), @QUYEN VARCHAR(50), @BANG VARCHAR(50)
AS
BEGIN
	IF(@QUYEN='ALL')
	BEGIN
		DELETE FROM PHANQUYEN WHERE TEN=@TEN AND BANG_CHON=@BANG
	END
	ELSE
	BEGIN
		DELETE FROM PHANQUYEN WHERE TEN=@TEN AND QUYEN=@QUYEN AND BANG_CHON=@BANG
	END
END

DROP PROC TT_XOA_PHANQUYEN
